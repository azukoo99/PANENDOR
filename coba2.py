...
def tambah_produk(data_produk):
    ...
    while True:
        try:
            nama_produk = input("Masukan nama produk: ").capitalize()
            stok_produk = float(input("Masukan stok produk: "))
            harga_produk = float(input("Masukan harga produk: "))
            keuntungan_produk =float(input("Masukan keuntungan produk: "))

            if (data_produk['produk'] == nama_produk).any():
    ...
    while True:
        try:
            ...
            if index_produk <= 0:
                ...
                input("Klik Enter untuk melanjutkan...")
                continue
            elif 0 <= index_produk < len(data_produk)+1:
...
def kelola_akun_kasir():
    os.system('cls')
    akun_kasir = pd.read_csv('user.csv',skiprows=[1])
    akun_kasir.index += 1
